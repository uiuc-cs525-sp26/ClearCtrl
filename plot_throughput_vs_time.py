...
OUT_TP_PNG = Path("analysis/throughput_vs_time.png")
OUT_TP_PDF = Path("analysis/throughput_vs_time.pdf")
...
def load_latest_run(path: Path) -> pd.DataFrame:
    df = pd.read_csv(path)
    for col in ["run_id", "phase_idx", "batch_payload_bytes", "batch_latency_ms"]:
...
def add_metrics(df: pd.DataFrame, phase_sleep_ms: dict[int, float]) -> pd.DataFrame:
    ...
    payload_mb = pd.to_numeric(df["batch_payload_bytes"], errors="coerce") / (1024.0 * 1024.0)
    inst_tp = payload_mb / (latency_ms / 1000.0).replace(0.0, pd.NA)

    out = df.copy()
    out["elapsed_s"] = elapsed
    out["inst_tp_mb_s_smooth"] = inst_tp.rolling(
        window=TP_SMOOTH_WINDOW,
        min_periods=max(3, TP_SMOOTH_WINDOW // 3),
    ).mean()
    return out
...
def main() -> None:
    ...
    ax.set_ylabel("Throughput (MB/s), rolling mean")
    ax.grid(alpha=0.25)
    ax.legend(loc="best", ncol=2, frameon=False)
    fig.tight_layout()

    OUT_TP_PNG.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(OUT_TP_PNG, dpi=220)
    fig.savefig(OUT_TP_PDF)
    plt.close(fig)

    print(f"[saved] {OUT_TP_PNG}")
    print(f"[saved] {OUT_TP_PDF}")
